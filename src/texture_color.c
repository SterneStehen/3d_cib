...
/*   Updated: 2024/09/04 14:11:57 by cpuiu            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	set_material_direction_and_wall_hit(t_game *game)
{
	...
	if (game->ray.collision_side == 0)
		game->material_data.wallHitX = game->ray.pos_y + game->ray.perpwalldist
			* game->ray.beam_dir_y;
	else
		game->material_data.wallHitX = game->ray.pos_x + game->ray.perpwalldist
			* game->ray.beam_dir_x;
}
...
static void	draw_wall_pixels(t_game *game)
{
	...
	{
		game->material_data.coordY = (int)game->material_data.position
		...
		if (j < game->resolut_height && game->ray.x < game->resolut_width)
		{
			game->render_data.buffer_pix[j * game->render_data.size_line / 4
				...
				.buffer_pix[game->material_data.coordY
...
static void	calculate_texture_mapping(t_game *game)
{
	...
	if (game->ray.collision_side == 0 && game->ray.beam_dir_x > 0)
		game->material_data.coordX = game->surfaces[game->material_data.dir]
			.win_width - game->material_data.coordX - 1;
	if (game->ray.collision_side == 1 && game->ray.beam_dir_y < 0)
		game->material_data.coordX = game->surfaces[game->material_data.dir]
			.win_width - game->material_data.coordX - 1;
