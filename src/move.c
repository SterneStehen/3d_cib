...
/*   Updated: 2024/09/04 23:37:46 by cpuiu            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	funk_a_d(t_game *game)
{
	...
	if (game->render_data.move_right == 1)
	{
		...
		new_pos_y = game->ray.pos_y - game->ray.dir_x * move_speed;
		...
		if (game->level_map[(int)game->ray.pos_x][(int)new_pos_y] == '0')
			game->ray.pos_y -= game->ray.dir_x * game->ray.move_speed;
	}
...
void	funk_w_s(t_game *game)
{
	...
	{
		new_pos_x = game->ray.pos_x + game->ray.dir_x * game->ray.move_speed;
		new_pos_y = game->ray.pos_y + game->ray.dir_y * game->ray.move_speed;
		if (game->level_map[(int)new_pos_x][(int)game->ray.pos_y] == '0')
			game->ray.pos_x += game->ray.dir_x * game->ray.move_speed;
		if (game->level_map[(int)game->ray.pos_x][(int)new_pos_y] == '0')
...
void	funk_left_right(t_game *game)
{
	...
	previos_x = game->ray.plan_x;
	previos_direct_x = game->ray.dir_x;
	if (game->render_data.right_rotet == 1 || game->render_data.left_rot == 1)
	{
		if (game->render_data.right_rotet == 1)
			...
			rotation_angle = game->ray.rotation_speed;
		game->ray.dir_x = game->ray.dir_x * cos(rotation_angle)
			- game->ray.dir_y * sin(rotation_angle);
		game->ray.dir_y = previos_direct_x * sin(rotation_angle)
			+ game->ray.dir_y * cos(rotation_angle);
		game->ray.plan_x = game->ray.plan_x * cos(rotation_angle)
			- game->ray.plan_y * sin(rotation_angle);
		game->ray.plan_y = previos_x * sin(rotation_angle) + game->ray.plan_y
			* cos(rotation_angle);
	}
}
