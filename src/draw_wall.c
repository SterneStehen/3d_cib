...
static void	calculate_ray_step_and_grid_intersection(t_game *game)
{
	...
	{
		game->ray.step_x = -1;
		game->ray.grid_x = (game->ray.pos_x - game->ray.mapX)
			* game->ray.dist_to_side_x;
	}
	else
	{
		game->ray.step_x = 1;
		game->ray.grid_x = (game->ray.mapX + 1.0 - game->ray.pos_x)
			* game->ray.dist_to_side_x;
	}
	if (game->ray.beam_dir_y < 0)
	{
		game->ray.step_y = -1;
		game->ray.grid_y = (game->ray.pos_y - game->ray.mapY)
			* game->ray.dist_to_side_y;
	}
	else
	{
		game->ray.step_y = 1;
		game->ray.grid_y = (game->ray.mapY + 1.0 - game->ray.pos_y)
			* game->ray.dist_to_side_y;
	}
}
...
void	draw_wall(t_game *game)
{
	...
	{
		game->ray.perpwalldist = ((double)game->ray.mapX - game->ray.pos_x + (1
					- (double)game->ray.step_x) / 2) / game->ray.beam_dir_x;
	}
	else
	{
		game->ray.perpwalldist = ((double)game->ray.mapY - game->ray.pos_y + (1
